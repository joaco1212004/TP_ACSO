...
int inode_iget(struct unixfilesystem *fs, int inumber, struct inode *inp) {
    ...
    int ips = DISKIMG_SECTOR_SIZE / sizeof(struct inode);  // 16 inodos/sector
    int total = fs->superblock.s_isize * ips;              // cantidad total de inodos
    ...
    *inp = buf[(inumber - 1) % ips];
    // Verifica que el inodo esté asignado
...
int inode_indexlookup(struct unixfilesystem *fs, struct inode *inp, int blockNum) {
    ...
    // Caso simple: direccionamiento directo
    if (!is_large) {
    ...
    // Esquema de archivos grandes
    int ptrs = DISKIMG_SECTOR_SIZE / sizeof(uint16_t);  // 256 punteros por sector
    // 1) Indirectos simples
    if (blockNum < 7 * ptrs) {
        ...
        if (diskimg_readsector(fs->dfd, iblk, buf) != DISKIMG_SECTOR_SIZE)
            return -1;
        int res = buf[off];
        return (res != 0 ? res : -1);
    }
    ...
    if (diskimg_readsector(fs->dfd, dbl, buf1) != DISKIMG_SECTOR_SIZE)
        return -1;
    ...
    if (diskimg_readsector(fs->dfd, iblk, buf2) != DISKIMG_SECTOR_SIZE)
        return -1;
    int res = buf2[off1];
    return (res != 0 ? res : -1);
}
